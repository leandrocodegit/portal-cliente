...
function CalledElement(props) {
  ...
  const [options, setOptions] = useState([
    { value: '', label: translate('<Selecionar um processo>') }
  ]);
    ...
    async function loadFormOptions() {
      try {
        ...
        const res = await fetch(
          'https://dev.simodapp.com:2083/processo/api/v1/process-definition?active=true&suspended=false&latestVersion=true',
          {
            method: 'GET',
            headers: {
...
function CalledElementVersion(props) {
  ...
  const [options, setOptions] = useState([
    { value: '', label: translate('<Selecionar a versão>') }
  ]);

  const calledElement = getBusinessObject(element).get('calledElement');

  useEffect(() => {
    if (!calledElement) {
      setOptions([{ value: '', label: translate('<Selecionar a versão>') }]);
      return;
    }
    ...
    async function loadFormOptions() {
      try {
        const token = sessionStorage.getItem("access_token");
        const res = await fetch(
          `https://dev.simodapp.com:2083/processo/api/v1/process-definition?key=${calledElement}&active=true&suspended=false&sortBy=version&sortOrder=desc`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        console.error('Erro ao carregar versões do processo', err);
      }
    }

    loadFormOptions();
  }, [calledElement, translate]);

  return jsx(SelectEntry, {
    element,
    id: "calledElementVersion",
    label: translate('Versão'),
    getValue,
    setValue,
    debounce,
    getOptions: () => options,
    disabled: !calledElement // 👈 desabilita se não houver processo selecionado
  });
}
  ...
  useEffect(() => {
    async function loadFormOptions() {
      try {
        ...
        // 👇 aqui assumo que sua API retorna uma lista de formulários:
        // [ { id: "form1", nome: "Formulário 1" }, ... ]
        const remoteOptions = data.map(f => ({
          value: f.key,
          label: f.nome
        }));
...
FormRef.isEdited = isSelectEntryEdited;
...
function CandidateUsers(props) {
  ...
  onfocus = () => {
    ...
    const cadidateGroups = document.getElementById('dialodBt');
    cadidateGroups.click();
  }
...
function DueDate(props) {
  ...
  return TextFieldEntry({
    ...
    id: 'dueDate',
    label: translate('Due date'),
    description: translate('A data de vencimento como uma expressão EL (ex.: ${someDate}) ou uma data no formato ISO (ex.: 2015-06-26T09:54:00).'),
    getValue,
    setValue,
    debounce
  });
...
function FollowUpDate(props) {
  ...
  return TextFieldEntry({
    ...
    id: 'followUpDate',
    label: translate('Follow up date'),
    description: translate('A data de acompanhamento como uma expressão EL (por exemplo, ${someDate}) ou uma data no formato ISO (por exemplo, 2015-06-26T09:54:00).'),
    getValue,
    setValue,
    debounce
  });
